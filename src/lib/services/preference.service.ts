...
export class PreferenceService {
    ...
    async getPreferences(userId: string): Promise<PreferenceDTO[]> {
        try {
            ...
            if (error) {
                throw new PreferenceError("Failed to fetch preferences", "DATABASE_ERROR", error);
            }
    ...
    async createPreference(userId: string, command: CreatePreferenceCommand): Promise<PreferenceDTO> {
        try {
            ...
            if (error) {
                ...
                throw new PreferenceError("Failed to create preference", "DATABASE_ERROR", error);
            }

            if (!data) {
                throw new PreferenceError("Failed to create preference - no data returned", "DATABASE_ERROR");
            }
    ...
    async updatePreference(
        ...
        try {
            ...
            if (error) {
                ...
                throw new PreferenceError("Failed to update preference", "DATABASE_ERROR", error);
            }

            if (!data) {
                throw new PreferenceError("Preference not found or access denied", "NOT_FOUND");
            }
    ...
    async deletePreference(userId: string, preferenceId: string): Promise<void> {
        try {
            ...
            if (error) {
                throw new PreferenceError("Failed to delete preference", "DATABASE_ERROR", error);
            }
        } catch (error) {
            if (error instanceof PreferenceError) {
                throw error;
