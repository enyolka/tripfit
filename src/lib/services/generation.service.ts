...
export class GenerationService {
    ...
    async getGenerationsForJourney(journeyId: number): Promise<GenerationDTO[]> {
        try {
            ...
            if (journeyError) {
                throw new GenerationError("Failed to fetch journey", "DATABASE_ERROR", journeyError);
            }

            if (!journey) {
                throw new GenerationError("Journey not found", "JOURNEY_NOT_FOUND");
            }
            ...
            if (generationsError) {
                throw new GenerationError("Failed to fetch generations", "DATABASE_ERROR", generationsError);
            }
            ...
            // Wrap unknown errors
            throw new GenerationError("Unexpected error occurred while fetching generations", "DATABASE_ERROR", error);
        }
    }
    ...
    async getGeneration(id: number): Promise<GenerationDTO> {
        try {
            ...
            if (error) {
                throw new GenerationError("Failed to fetch generation", "DATABASE_ERROR", error);
            }
    ...
    async updateGeneration(generationId: number, command: UpdateGenerationCommand): Promise<GenerationDTO> {
        try {
            ...
            if (getError || !currentGeneration) {
                throw new GenerationError("Generation not found or access denied", "GENERATION_NOT_FOUND");
            }
            ...
            if (updateError) {
                throw new GenerationError("Failed to update generation", "DATABASE_ERROR", updateError);
            }
    ...
    async deleteGeneration(generationId: number): Promise<void> {
        try {
            ...
            const { error: deleteError } = await this.supabase.from("generations").delete().eq("id", generationId);

            if (deleteError) {
                throw new GenerationError("Failed to delete generation", "DATABASE_ERROR", deleteError);
            }
        } catch (error) {
            if (error instanceof GenerationError) {
                throw error;
