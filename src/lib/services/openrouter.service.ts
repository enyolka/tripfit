...
export class OpenRouterService {
    ...
    constructor(config: Partial<ModelConfig> = {}) {
        ...
        if (!this.apiKey) {
            throw new OpenRouterError("OpenRouter API key is not configured", "UNEXPECTED_ERROR");
        }
    }

    private validateInput(payload: ChatPayload): void {
        if (!Array.isArray(payload.messages) || payload.messages.length === 0) {
            throw new OpenRouterError("Messages array must not be empty", "VALIDATION_ERROR");
        }

        for (const message of payload.messages) {
            if (!message.content || typeof message.content !== "string") {
                throw new OpenRouterError("Each message must have a content string", "VALIDATION_ERROR");
            }
            if (!["system", "user"].includes(message.role)) {
                throw new OpenRouterError('Message role must be either "system" or "user"', "VALIDATION_ERROR");
            }
        }

        if (payload.responseFormat) {
            if (payload.responseFormat.type !== "json_schema" || !payload.responseFormat.schema) {
                throw new OpenRouterError("Invalid response format configuration", "VALIDATION_ERROR");
            }
        }
    }
    ...
    public setModelConfig(config: Partial<ModelConfig>): void {
        ...
        if (config.model && typeof config.model !== "string") {
            throw new OpenRouterError("Model name must be a string", "VALIDATION_ERROR");
        }

        // Validate maxTokens if provided
        if (config.maxTokens !== undefined) {
            if (!Number.isInteger(config.maxTokens) || config.maxTokens <= 0) {
                throw new OpenRouterError("maxTokens must be a positive integer", "VALIDATION_ERROR");
            }
        }
        ...
        if (config.temperature !== undefined) {
            if (typeof config.temperature !== "number" || config.temperature < 0 || config.temperature > 1) {
                throw new OpenRouterError("temperature must be a number between 0 and 1", "VALIDATION_ERROR");
            }
        }
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 429) {
                        const rateLimitError = new OpenRouterError("Rate limit exceeded", "RATE_LIMIT_ERROR", error);
                ...
                if (!content) {
                    const error = new OpenRouterError("Invalid response format from API", "API_ERROR", data);
                    this.logger.logRequestError(
                        requestId,
                        this.modelConfig.model,
                        payload.messages?.length || 0,
                ...
                if (!(error instanceof OpenRouterError)) {
                    ...
                    throw new OpenRouterError("Unexpected error during API request", "UNEXPECTED_ERROR", error);
                }
                throw error;
            }
        });
