    ...
    try {
        ...
        try {
            ...
            if (error instanceof RateLimitError) {
                return new Response(JSON.stringify({ error: "Rate limit exceeded", message: error.message }), {
                    status: 429,
                });
            }
            throw error;
        ...
        if (journeyError || !journey) {
            return new Response(JSON.stringify({ error: "Journey not found or access denied" }), { status: 404 });
        }

        const aiService = new AIService(model_config);

        try {
            const { generatedText, model, sourceTextHash, sourceTextLength } = await aiService.generateTravelPlan(
                journey,
                plan_preferences
            );
