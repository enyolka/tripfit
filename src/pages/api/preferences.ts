...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const preferenceService = new PreferenceService(locals.supabase as SupabaseClient);
    const newPreference = await preferenceService.createPreference(user.id, validationResult.data as CreatePreferenceCommand);
  ...
  try {
    ...
    if (error instanceof PreferenceError) {
      switch (error.code) {
        case 'NOT_FOUND':
          return new Response(
            JSON.stringify({ error: "Preference not found or access denied" }), 
            { 
              status: 404,
              headers: { 'Content-Type': 'application/json' }
            }
