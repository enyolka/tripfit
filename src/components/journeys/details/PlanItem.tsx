...
export function PlanItem({ plan, onUpdate, onDeleteRequest, isUpdating }: PlanItemProps) {
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <div className="space-y-1">
                    <div id={headingId} className="text-sm text-muted-foreground">
                        ...
                        {plan.edited_at && <span className="block">Last edited on {formatDate(plan.edited_at)}</span>}
                    </div>
                </div>
                <PlanActions
                    isEditing={isEditing}
